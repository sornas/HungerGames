        ...
        if (args.length == 0 || !plugin.getCommands().containsKey(args[0])) {
            ...
            for (BaseCmd cmd : plugin.getCommands().values().toArray(new BaseCmd[0])) {
                if (s.hasPermission("hg." + cmd.cmdName))
                    Util.scm(s, "  &7&l- " + cmd.sendHelpLine());
            }
            Util.scm(s, "&4*&c&m                                                                             &4*");
        } else
            plugin.getCommands().get(args[0]).processCmd(plugin, s, args);
        return true;
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String s, String[] args) {
        if (args.length == 1) {
            ...
            if (args[0].equalsIgnoreCase("team") && sender instanceof Player) {
                ...
                return null;
            } else if (args[0].equalsIgnoreCase("delete") || args[0].equalsIgnoreCase("debug")
                    || args[0].equalsIgnoreCase("stop") || (args[0].equalsIgnoreCase("forcestart"))
                    || (args[0].equalsIgnoreCase("join")) || (args[0].equalsIgnoreCase("setlobbywall"))
                    || (args[0].equalsIgnoreCase("toggle")) || (args[0].equalsIgnoreCase("bordercenter"))
