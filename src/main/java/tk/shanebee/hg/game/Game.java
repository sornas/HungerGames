...
public class Game {
     ...
     * Create a new game
     * <p>
     * Internally used when loading from config on server start
     * </p>
     * 
     ...
     */
    public Game(String name, Bound bound, List<Location> spawns, Sign lobbySign, int timer, int minPlayers,
            int maxPlayers, int roam, boolean isReady, int cost) {
     ...
     * Create a new game
     * <p>
     * Internally used when creating a game with the <b>/hg create</b> command
     * </p>
     * 
     * @param name       Name of this game
     * @param bound      Bounding region of this game
     ...
     * <p>
     * <b>format = </b> "type:command"
     * </p>
     * <p>
     * <b>types = </b> start, stop, death, join
     * </p>
     * 
     ...
     * 
     * @param location Location of the chest to add (Needs to actually be a chest
     *                 there)
     */
        ...
        if (status != Status.WAITING && status != Status.STOPPED && status != Status.COUNTDOWN
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(HG.getPlugin(), () -> {
                ...
                if (players.size() >= minPlayers && (status == Status.WAITING || status == Status.READY)) {
                    ...
                    Util.broadcast(HG.getPlugin().getLang().player_joined_game.replace("<player>", player.getName())
                            + (minPlayers - players.size() <= 0 ? "!"
                                    : ":" + HG.getPlugin().getLang().players_to_start.replace("<amount>",
                                            String.valueOf((minPlayers - players.size())))));
                }
                kitHelp(player);
    ...
    public void startGame() {
        ...
        if (Config.spawnmobs)
            spawner = new SpawnerTask(this, Config.spawnmobsinterval);
    ...
    public boolean setLobbyBlock(Sign sign) {
        try {
            ...
            if (cost > 0)
                s.setLine(3, Util.getColString(
                        HG.getPlugin().getLang().lobby_sign_cost.replace("<cost>", String.valueOf(cost))));
        ...
        try {
            ...
            this.exit = new Location(Bukkit.getServer().getWorld(h[0]), Integer.parseInt(h[1]) + 0.5,
                    Integer.parseInt(h[2]) + 0.1, Integer.parseInt(h[3]) + 0.5, Float.parseFloat(h[4]),
     ...
     * Stop the game
     * 
     * @param death Whether the game stopped after the result of a death (false = no
     *              winnings payed out)
     */
    ...
    /*
     * ON HOLD FOR NOW private boolean isGameOver() { if (players.size() <= 1)
     * return true; for (Entry<UUID, PlayerData> f :
     * HG.getPlugin().getPlayers().entrySet()) {
     * 
     * Team t = f.getValue().getTeam();
     * 
     * if (t != null && (t.getPlayers().size() >= players.size())) { List<UUID> ps =
     * t.getPlayers(); for (UUID u : players) { if (!ps.contains(u)) { return false;
    ...
    private void createBossbar(int time) {
        int min = (time / 60);
        int sec = (time % 60);
        String title = HG.getPlugin().getLang().bossbar.replace("<min>", String.valueOf(min)).replace("<sec>",
                String.valueOf(sec));
        bar = Bukkit.createBossBar(ChatColor.translateAlternateColorCodes('&', title), BarColor.GREEN,
    ...
    public void runCommands(CommandType commandType, @Nullable Player player) {
        ...
        for (String command : commands) {
            ...
            if (command.equalsIgnoreCase("none"))
                continue;
            command = command.split(":")[1].replace("<world>", this.bound.getWorld().getName()).replace("<arena>",
                    this.getName());
            if (player != null) {
    ...
    public String toString() {
        return "Game{" + "name='" + name + '\'' + ", bound=" + bound + '}';
    }
