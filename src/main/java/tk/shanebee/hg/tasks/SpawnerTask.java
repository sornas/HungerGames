...
public class SpawnerTask implements Runnable {
     ...
     * private EntityType pickRandomMob(boolean isNight, int x) { if (isNight) { if
     * (x < 3) return EntityType.ZOMBIE; if (x < 5) return EntityType.DROWNED; if (x
     * < 7) return EntityType.SKELETON; if (x < 8) return EntityType.STRAY;
     * 
     * return EntityType.CREEPER; } else { if (x < 2) return EntityType.COW; if (x <
     * 5) return EntityType.PIG; if (x < 6) return EntityType.CHICKEN; if (x < 7)
     * return EntityType.SHEEP; if (x < 8) return EntityType.PARROT; if (x < 9)
